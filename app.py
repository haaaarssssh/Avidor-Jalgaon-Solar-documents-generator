...
from docx.shared import Inches, Pt  # Added Pt
import os
import zipfile
...
from PIL import Image as PILImage # Added for explicit usage in helper
import base64
import subprocess
import re
...
def add_image_with_aspect_ratio(run, image_path, target_width_inches=1.0):
    """
    Add image to run while preserving aspect ratio to prevent stretching in PDF
    """
    try:
        ...
        target_height = Inches(target_width_inches * aspect_ratio)
        
        # Add picture with both width and height
        run.add_picture(image_path, width=target_width, height=target_height)
        print(f"   ✓ Image added: {target_width_inches}\" wide, aspect ratio {aspect_ratio:.2f}")
        
    except Exception as e:
        print(f"   ✗ Error adding image with aspect ratio: {e}")
        # Fallback to width-only
        run.add_picture(image_path, width=Inches(target_width_inches))

def add_vendor_stamp_to_document(doc, stamp_path, target_width=1.5):
    """
    Add vendor stamp to specific locations in document (searches for markers)
    """
    try:
        if not os.path.exists(stamp_path):
            # Try absolute path if relative fails
            if not os.path.isabs(stamp_path):
                stamp_path = os.path.join(app.root_path, stamp_path)
        ...
        stamp_added = False
        # Markers to look for
        stamp_markers = ['[Vendor]', 'Stamp & Seal', 'DIRECTOR', 'stamp_image_variable']
        
        # Strategy 1: Check paragraphs
        for para in doc.paragraphs:
            para_text = para.text
            for marker in stamp_markers:
                if marker.lower() in para_text.lower():
                    run = para.add_run()
                    run.add_text('\n')  # Add line break
                    add_image_with_aspect_ratio(run, stamp_path, target_width_inches=target_width)
                    stamp_added = True
                    print(f" ✓ Vendor stamp added after '{marker}'")
                    break
                    ...
                    for para in cell.paragraphs:
                        ...
                        for marker in stamp_markers:
                            if marker.lower() in para_text.lower():
                                ...
                                add_image_with_aspect_ratio(run, stamp_path, target_width_inches=target_width)
                                stamp_added = True
                                print(f" ✓ Vendor stamp added in table after '{marker}'")
                                break
        
        return stamp_added
        
    except Exception as e:
        print(f" ✗ Error adding vendor stamp: {e}")
        traceback.print_exc()
        return False
    ...
    for text, is_replacement in segments:
        if text:
            ...
            if original_font_size:
                sz = OxmlElement('w:sz')
                sz.set(qn('w:val'), str(int(original_font_size.pt * 2)))
                rPr.append(sz)
...
def add_images_to_wcr(doc, aadhar_path, signature_path):
    """Add consumer signature and aadhar images with ASPECT RATIO FIX"""
    try:
        replaced_count = 0
        for para in doc.paragraphs:
            para_text = para.text
                ...
                for run in para.runs:
                    if 'signature_image_variable' in run.text:
                        ...
                        if signature_path and os.path.exists(signature_path):
                            # UPDATED: Use aspect ratio helper
                            add_image_with_aspect_ratio(run, signature_path, target_width_inches=1.0)
                            replaced_count += 1
                ...
                for run in para.runs:
                    if 'aadhar_image_variable' in run.text:
                        ...
                        if aadhar_path and os.path.exists(aadhar_path):
                            # UPDATED: Use aspect ratio helper
                            add_image_with_aspect_ratio(run, aadhar_path, target_width_inches=2.0)
...
def add_signature_to_proforma(doc, signature_path):
    """Replace signature_image_variable text with actual signature image with ASPECT RATIO FIX"""
            ...
            if 'signature_image_variable' in para.text:
                for run in para.runs:
                    if 'signature_image_variable' in run.text:
                        run.text = run.text.replace('signature_image_variable', '')
                        # UPDATED: Use aspect ratio helper
                        add_image_with_aspect_ratio(run, signature_path, target_width_inches=0.8)
                        replaced_count += 1
...
def generate_documents():
    ...
    try:
        ...
        # Path to your hardcoded stamp
        STAMP_PATH = os.path.join('static', 'images', 'stamp.jpg')
        ...
        for doc_name, template_path in DOCUMENT_TEMPLATES.items():
            ...
            try:
                ...
                if doc_name == 'WCR':
                    ...
                    # Add Hardcoded Stamp to WCR
                    add_vendor_stamp_to_document(doc, STAMP_PATH, target_width=1.5)
                ...
                if doc_name == 'Proforma-A':
                    ...
                    # Add Hardcoded Stamp to Proforma-A
                    add_vendor_stamp_to_document(doc, STAMP_PATH, target_width=1.5)
                
                # You can enable stamp for other docs too if needed:
                if doc_name == 'Model-Agreement' or doc_name == 'NET':
                     add_vendor_stamp_to_document(doc, STAMP_PATH, target_width=1.5)
